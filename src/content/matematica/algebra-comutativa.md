...
- **Anéis Cohen-Macaulay**: anel local Noetheriano (A, 𝔪) é Cohen-Macaulay se depth(A) = dim(A), onde depth = comprimento máximo de sequência regular em 𝔪. Geometricamente: ausência de singularidades "patológicas" — variedades CM têm propriedades como Serre duality e fórmula de Hilbert-Samuel limpa. Anéis regulares ⊂ Cohen-Macaulay ⊂ Gorenstein ⊂ Cohen-Macaulay (a primeira é igualdade trivial; Gorenstein é CM com módulo dualizing livre de posto 1). Importante: hipersuperfícies (k[x₁,...,xₙ]/(f)) são Cohen-Macaulay; interseções completas também.
- **Localização como germes**: a localização A_P para P primo é precisamente "o anel de germes de funções regulares em P". Geometricamente: se Spec(A) = X e P corresponde ao ponto x ∈ X, então A_P = stalk do sheaf 𝒪_X em x. Resultados locais (regularidade, dimensão, profundidade) são propriedades de A_P; globais são de A. Essa dicotomia local/global é a base da geometria algébrica via esquemas.
- **Bases de Groebner (algorítmica)**: para ideal I ⊂ k[x₁,...,xₙ] e ordem monomial ≤ (lex, grlex, grevlex), base de Groebner G = {g₁,...,gᵣ} satisfaz: ⟨LT(g₁),...,LT(gᵣ)⟩ = ⟨LT(I)⟩ (leading terms). Algoritmo de Buchberger: começa com geradores, computa S-polinômios e reduz; termina por Noetherianidade. Aplicações: decisão de pertinência (f ∈ I?), eliminação de variáveis (interseção com k[x₁,...,xₖ]), dimensão e Hilbert series. Complexidade: pior caso doubly-exponential em n; na prática F4/F5 (Faugère) é prático.
- **Decomposição primária**: para A Noetheriano e I ideal, existe decomposição I = Q₁ ∩ ... ∩ Qᵣ com Qᵢ primários (Qᵢ tem único primo associado Pᵢ = √Qᵢ). Análogo de fatoração única em primos para ideais; única a menos de "primos imersos" (associados não-minimais). Computável via Groebner; corresponde a decomposição de V(I) em componentes irredutíveis + estrutura de multiplicidade.
