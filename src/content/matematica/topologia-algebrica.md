...
- **Grupos de homotopia superiores πₙ**: πₙ(X, x₀) = classes de homotopia de mapas (Sⁿ, *) → (X, x₀). π₀ conta componentes; π₁ é o grupo fundamental; πₙ para n ≥ 2 é sempre abeliano (Eckmann-Hilton). Exemplos: πₙ(Sⁿ) = ℤ (grau); π₃(S²) = ℤ (fibração de Hopf — surpreendentemente não-trivial!); πₙ(Sᵐ) para n ≤ m são difíceis de calcular em geral. **Grupos estáveis de homotopia** π^s_n = lim πₙ₊ₖ(Sᵏ) (estabilização): conhecidos só finitamente até n ~ 60 (Adams, Ravenel, Hill-Hopkins-Ravenel).
- **Cohomologia de sheaves**: sheaf ℱ em X = atribuição contínua de "dados locais" (grupos abelianos, anéis, módulos) a abertos U ⊂ X, com restrições compatíveis e colagem (axiomas de presheaf + sheaf). Exemplos: sheaf de funções contínuas C_X, sheaf de seções de fibrado vetorial, sheaf constante ℤ. Cohomologia Hⁱ(X, ℱ) generaliza cohomologia singular — para sheaf constante, recupera Hⁱ_{sing}. Calcula obstrução para passar de propriedades locais a globais (ex: H¹(X, 𝒪_X*) = grupo de Picard = classes de fibrados lineares). Computacionalmente: resolução de Čech ou injetivas; sequência espectral de Leray para mapas contínuos. Pré-requisito para geometria algébrica moderna (Grothendieck, Serre).
- **Classes características**: invariantes de fibrados vetoriais que medem "torcimento" topológico. Para fibrado complexo E → X: classes de Chern cᵢ(E) ∈ H^{2i}(X; ℤ); para real orientado: Pontryagin pᵢ(E) ∈ H^{4i}(X; ℤ); para qualquer real: Stiefel-Whitney wᵢ(E) ∈ H^i(X; ℤ/2). Total Chern c(E) = 1 + c₁ + c₂ + ...; c(E ⊕ F) = c(E) · c(F) (Whitney). Aplicações: invariantes de variedades (assinatura), obstrução a estrutura (espinor), índice de operadores elípticos (Atiyah-Singer).
- **Espectros e teoria de homotopia estável**: espectro = sequência {Eₙ} de espaços pontuais com mapas estrutura ΣEₙ → Eₙ₊₁. Generaliza espaços; equivalência por homotopia estável. Suspensão Σ é "invertível" em espectros. Espectro de esferas 𝕊 dá grupos estáveis π_*^s; espectros como K-theory, MU (cobordismo complexo) cobram informação fina. Categoria estável é categoria triangulada — refletiu na invenção de categorias derivadas e ∞-categorias.
