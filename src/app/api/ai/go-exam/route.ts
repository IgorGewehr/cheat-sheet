...
import { getExam, type ExamQuestion } from "@/lib/go-exam-questions";
...
export interface QuestionEvaluation {
  ...
  score: number; // 0-10
  feedback: string;
  expectedPoints: string[]; // pontos que faltaram (vazio se PASS)
}

export interface ExamEvaluation {
  tier: 1 | 3 | 5;
  finalScore: number; // 0-100, ponderado pelos weights
  passed: boolean;
  threshold: number;
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `Você é um entrevistador sênior em Go avaliando o exame Tier ${exam.tier} de um candidato (alvo: ${exam.title}). Seja rigoroso, técnico e honesto. Use a rubrica de cada questão como gabarito — não invente critérios.
...
Retorne JSON com EXATAMENTE esta estrutura:
{
  ...
  "questoes": [
    ...
    console.error("[go-exam] erro:", err);
...
export async function GET() {
  return NextResponse.json({
    description: "POST tier (1|3|5) e answers para avaliação do exame Go-Enterprise.",
    body: {
      tier: "1 | 3 | 5",
