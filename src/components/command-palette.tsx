...
export function CommandPalette({ cards, projects = [] }: Props) {
  ...
  const trigger = (
    <button
      ...
      className="w-full flex items-center gap-2 px-3 py-2 text-xs rounded-lg border border-line text-muted hover:text-fg hover:border-amber-500/40 hover:bg-card-hover transition"
      title="Buscar (Cmd+K)"
    >
      <Search className="w-3.5 h-3.5" />
      <span className="flex-1 text-left">Buscar…</span>
      <kbd className="px-1.5 py-0.5 rounded bg-card text-[10px] font-mono text-subtle">⌘K</kbd>
    </button>
  );
